...
recipe_seq <- recipe(type ~ ., data=train) %>%
    ...
    step_rm(-id,-starts_with("ratio"),-type,-starts_with("zscore"),-starts_with("cov")) %>%
...
recipe_seq <- recipe(type ~ ., data=train) %>%
    ...
    step_pca(starts_with("ratio"), prefix = "ratio_pc_",num_comp=5)     %>%
    step_corr(all_predictors(), threshold=0.95) %>%
    step_nzv(all_predictors())
...
#SSL 5 - LTR
model_name<-"LTR_ssl"
e<-fread("/dcs04/scharpf/data/annapragada/Brain/ARTEMIS_Novaseq/Expected.csv")
